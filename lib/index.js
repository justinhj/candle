...
export const sparkey_returncode = {
  SPARKEY_SUCCESS: 0,
...
export const sparkey_iterator_state = {
  SPARKEY_ITER_NEW: 0,
...
export async function sparkey_openHash(index_file_path, log_file_path) {
  let reader = {};
...
function logiter_skip(iter, log, count) {
  while(count > 0) {
    count--;
    let rc = sparkey_logiter_next(iter, log);
    if(rc !== sparkey_returncode.SPARKEY_SUCCESS) {
      throw new Error(rc);
    }
  }
...
export function sparkey_logiter_next(iter, log) {
...
export function sparkey_logiter_keychunk(iter, log, maxlen) {
  return logiter_chunk(iter, log, maxlen, BigInt(iter.key_remaining));
}

export function sparkey_logiter_valuechunk(iter, log, maxlen) {
...
export function sparkey_get(reader, log_iterator, lookupKeyBuf, valueBuffer) {
  ...
  //     RETHROW(sparkey_logiter_skip(iter, &reader.log, entry_index2));
      ...
      rc = sparkey_logiter_next(log_iterator, reader.log);
      if(rc !== sparkey_returncode.SPARKEY_SUCCESS) {
        throw new Error(rc);
      }
      ...
      if (keylen === keylen2) {
        ...
        while (pos2 < keylen) {
          let result = sparkey_logiter_keychunk(log_iterator, reader.log, keylen); 
        ...
        if(equals) {
          if(log_iterator.state === sparkey_iterator_state.SPARKEY_ITER_ACTIVE) {
            ...
            let pos = 0n;
            do {
              result = sparkey_logiter_valuechunk(log_iterator, reader.log, reader.log.header.max_value_len);
...
export async function sparkey_closeHash(reader) {
  await closeLog(reader.log);
  reader.log = null;
  reader.open_status = null;
}

export function sparkey_logiter_create(log) {
  assert_log_open(log);
...
export function sparkey_logiter_close(iter) {
  if(iter.open_status !== MAGIC_VALUE_LOGITER) {
    return;
  }
  iter.open_status = 0;
